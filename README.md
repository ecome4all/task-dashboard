...
- **Repeating tasks** (`RecurringTask`) — "Repeat this" on any task (admin/manager) copies it into a standalone repeat set to daily/weekly/monthly. It's a **copy, not a reference**: editing, completing or deleting the original afterwards changes nothing about what the repeat produces. Triage (employee/type/marketplace) is carried across, so a repeat doesn't come back stripped. Manage them on the **Repeating Tasks** screen. First run is always one interval out, so setting up a weekly repeat doesn't instantly duplicate the task you're looking at. **Setting up a second repeat with the same wording for the same person is refused** (`services/repeatDuplicates.ts`, 409) — because a repeat is a copy, the board can't show that a task already repeats, so "Repeat" looks as unpressed the second time as the first; two repeats side by side then produce two identical tasks a second apart on every run, which reads as a bug in task creation rather than as two repeats. The check ignores case and spacing, is made against all repeats rather than the visible ones, and doesn't count stopped repeats.
