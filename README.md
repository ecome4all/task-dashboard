...
- **Send Report — removed (2026-08-10).** It composed a metrics update by hand, one pasted row per product, and could attach a saved report link into the message. Every client now has a sheet linked, so Reports reads the figures instead of anyone pasting them, and two screens that both sent to clients was one too many to reason about. Its `/api/report-links` routes, the `ReportLink` table and the `api.ts` helpers are all still there and untouched — no data was deleted, and `git revert` of the removal brings the screen back. Strip those too only once it's certain no one wants it.
